...
class CosmAPI
{
	...
	{
		$url = "http://$this->Cosm/feeds/$feed/datastreams";
	...
	public function getDatastream($format=false, $feed, $datastream)
	{
		...
		$url = "http://$this->Cosm/feeds/$feed/datastreams/$datastream";
		return $this->_getRequest($url);
	}
	...
	/**
	 * Create GET request to Cosm (wrapper)
	 * @param string url
	...
	/**
	 * Create POST request to Cosm (wrapper)
	 * @param string url
	 * @param array data
	 * @return http code response
	 */
	...
	/**
	 * Create PUT request to Cosm (wrapper)
	 * @param string url
	 * @param string data
	 * @return http code response
	 */
	...
	{	
		if(function_exists('curl_init'))
		{
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, $this->Cosm_headers);
	...
	private function _curl($url, $auth=false, $post=false, $post_data=false)
	{
		if(function_exists('curl_init'))
		{
			...
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			if($auth) curl_setopt($ch, CURLOPT_HTTPHEADER, $this->Cosm_headers);
			if($post)
			{
	...
	public function _debugStatus($status_code)
	{
		...
		{			
			case 200:
				$msg = "Cosm feed successfully updated";	
				break;
			case 401:
				$msg = "Cosm API key was incorrect";
				break;
			case 403:
